...
all_metrics <- purrr::map2_dfr(list(lm_all_add, lm_all_pairs_cont, lm_cat_add, lm_cust_X_sig_splines, 
                                    lm_cont_add_sig_splines, lm_cont_X_sig_splines, 
...
RMSE_vector <- c(sqrt(mean(lm_all_add$residuals^2)),
                  ...
                  sqrt(mean(lm_cont_add_sig_splines$residuals^2)),                                                                                                        sqrt(mean(lm_cont_X_sig_splines$residuals^2)),
...
Looking at the performance above, the best performer by AIC/BIC is *lm_all_add* (All Additive). Additionally I would say *lm_cont_x_region* also performed well with a good R-squared. As far as my choice, I think *lm_cont_add_sig_splines* performed the best. With one of the lower AIC/BIC and still a decent Rsquared. *lm_cont_x_region* also performed very well, and would be 3rd place if not considering the ones I made.

**Coefficient Summaries**

Now we will look at the coefficients of the top 3. 
```{r, coefficient of top 1}
...
coefplot(lm_cont_add_sig_splines)
```
